...
def _tower_loss(iterator, num_of_classes, ignore_label, scope, reuse_variable):
  ...
  with tf.compat.v1.variable_scope(
      tf.compat.v1.get_variable_scope(), reuse=True if reuse_variable else None):
  ...
  regularization_loss = tf.compat.v1.losses.get_regularization_loss(scope=scope)
  tf.compat.v1.summary.scalar('Losses/%s' % regularization_loss.op.name,
                    regularization_loss)
...
def _average_gradients(tower_grads):
  ...
  for grad_and_vars in zip(*tower_grads):
    ...
    grad = tf.reduce_mean(input_tensor=tf.stack(grads, axis=0), axis=0)
...
def _log_summaries(input_image, label, num_of_classes, output):
  ...
  if FLAGS.save_summaries_images:
    tf.compat.v1.summary.image('samples/%s' % common.IMAGE, input_image)
    ...
    predictions = tf.expand_dims(tf.argmax(input=output, axis=3), -1)
    summary_predictions = tf.cast(predictions * pixel_scaling, tf.uint8)
    tf.compat.v1.summary.image('samples/%s' % common.OUTPUT_TYPE, summary_predictions)
...
def _train_deeplab_model(iterator, num_of_classes, ignore_label):
  ...
  for i in range(FLAGS.num_clones):
    with tf.device('/gpu:%d' % i):
      ...
      with tf.compat.v1.name_scope(name_scope) as scope:
        loss = _tower_loss(
            iterator=iterator,
  ...
  with tf.device('/cpu:0'):
    ...
    total_loss = tf.cond(
        pred=should_log,
        true_fn=lambda: tf.compat.v1.Print(total_loss, [total_loss], 'Total loss is :'),
        false_fn=lambda: total_loss)
    ...
    summary_op = tf.compat.v1.summary.merge_all(scope='(?!clone_)')
...
def main(unused_argv):
  ...
  with graph.as_default():
    with tf.device(tf.compat.v1.train.replica_device_setter(ps_tasks=FLAGS.num_ps_tasks)):
